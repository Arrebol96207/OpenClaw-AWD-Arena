...
class _FakeNetworks:
    def __init__(self, create_result=None, get_result=None, create_error=None, get_error=None):
...
def test_create_target_container_passes_restart_policy_and_records_container_id(monkeypatch):
    ...
    assert len([key for key in run_call["kwargs"]["environment"] if key.startswith("FLAG_")]) == 5
